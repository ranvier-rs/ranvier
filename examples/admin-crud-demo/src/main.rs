...
//!
//! ## Purpose
//! Bridge example showing an admin-style backend with JWT login, SQLite CRUD,
//! pagination/search, and OpenAPI on top of explicit Axon pipelines.
//!
//! ## Run
//! ```bash
...
//!
//! ## Key Concepts
//! - SQLite resource injection with `SqlitePool`
//! - JWT-gated admin routes using explicit header extraction
//! - Typed JSON input/output with `post_typed_json_out` and `get_json_out`
//! - OpenAPI/Swagger generation from Ranvier route descriptors
//!
//! ## Prerequisites
//! - `reference-todo-api` — CRUD + auth baseline
//! - `openapi-demo` — OpenAPI generation baseline
//!
//! ## Next Steps
//! - `reference-fullstack-admin` — public-only fullstack reference app
...
async fn initialize_db(pool: &SqlitePool) -> Result<(), String> {
    ...
    let dept_count_row = sqlx::query("SELECT COUNT(*) AS count FROM departments")
        .fetch_one(pool)
        .await
        .map_err(|e| format!("count departments failed: {e}"))?;
    let dept_count: i64 = dept_count_row
        .try_get("count")
...
impl Transition<(), UserPage> for ListUsers {
    ...
    async fn run(&self, _input: (), resources: &Self::Resources, bus: &mut Bus) -> Outcome<UserPage, Self::Error> {
        ...
        let department_id: Option<i64> = bus.query_param("department_id");
        ...
        let q_like = q_trimmed.as_ref().map(|value| format!("%{value}%"));

        let total = match (q_like.as_ref(), department_id) {
            (Some(q_value), Some(dept_id)) => {
                match sqlx::query(
                    r#"
            ...
            (None, Some(dept_id)) => {
                match sqlx::query("SELECT COUNT(*) AS count FROM users WHERE department_id = ?")
                    .bind(dept_id)
                    .fetch_one(&resources.pool)
                    .await
                {
            ...
            (None, None) => match sqlx::query("SELECT COUNT(*) AS count FROM users")
                .fetch_one(&resources.pool)
                .await
            {
                Ok(row) => match row.try_get::<i64, _>("count") {
        ...
        let rows = match (q_like.as_ref(), department_id) {
            (Some(q_value), Some(dept_id)) => {
                sqlx::query(
                    r#"
                    SELECT
                    ...
                    WHERE users.department_id = ?
                      AND (users.username LIKE ? OR users.full_name LIKE ? OR users.email LIKE ?)
...
#[async_trait]
impl Transition<(), UserRecord> for GetUser {
    type Error = String;
    type Resources = AppState;
    ...
    async fn run(&self, _input: (), resources: &Self::Resources, bus: &mut Bus) -> Outcome<UserRecord, Self::Error> {
        ...
        match fetch_user_by_id(&resources.pool, id).await {
            ...
            Ok(None) => Outcome::Fault(format!("user not found: {id}")),
            Err(error) => Outcome::Fault(error),
        }
    }
}
...
impl Transition<CreateUserInput, UserRecord> for CreateUser {
    ...
    async fn run(&self, input: CreateUserInput, resources: &Self::Resources, bus: &mut Bus) -> Outcome<UserRecord, Self::Error> {
        ...
        if input.username.trim().is_empty() {
            return Outcome::Fault("username cannot be empty".to_string());
        }
        if input.full_name.trim().is_empty() {
            return Outcome::Fault("full_name cannot be empty".to_string());
        }
        if input.email.trim().is_empty() {
            return Outcome::Fault("email cannot be empty".to_string());
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let addr = std::env::var("BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:3120".to_string());
    ...
    let ingress = Ranvier::http::<AppState>()
        ...
        .get_json_out("/users/:id", get_user)
    ...
    let openapi_json = OpenApiGenerator::from_ingress(&ingress)
        .title("Ranvier Admin CRUD Demo")
        .version("0.46.0-dev")
        .description("Bridge example: admin-style JWT CRUD backend with SQLite, search, pagination, and OpenAPI.")
        .with_bearer_auth()
        .summary(Method::POST, "/login", "Issue JWT for the admin demo user")
        .summary(Method::GET, "/departments", "List departments")
        .summary(Method::GET, "/users", "List users with pagination and search")
        .summary(Method::GET, "/users/:id", "Get a user by id")
        .summary(Method::POST, "/users", "Create a user")
        .summary(Method::PUT, "/users/:id", "Update a user")
        .summary(Method::DELETE, "/users/:id", "Delete a user")
        ...
        .json_response_schema::<UserRecord>(Method::GET, "/users/:id")
    ...
    let state = AppState {
        pool,
        openapi_json,
        swagger_html: swagger_ui_html("/openapi.json", "Ranvier Admin CRUD Demo"),
    };

    println!("Admin CRUD Demo listening on http://{addr}");
    println!("  POST /login");
    println!("  GET  /departments");
    println!("  GET  /users");
    println!("  POST /users");
