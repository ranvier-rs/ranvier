...
impl Transition<SagaState, SagaState> for SuccStep {
    type Error = String;
    type Resources = ();
    fn label(&self) -> String { format!("Step_{}", self.id) }
    async fn run(&self, mut state: SagaState, _res: &(), _bus: &mut Bus) -> Outcome<SagaState, String> {
        state.cnt += 1;
...
impl Transition<SagaState, ()> for CompStep {
    type Error = String;
    type Resources = ();
    fn label(&self) -> String { format!("Comp_{}", self.id) }
...
impl Transition<SagaState, SagaState> for FailStep {
    type Error = String;
    type Resources = ();
    fn label(&self) -> String { "FailStep".to_string() }
    async fn run(&self, _state: SagaState, _res: &(), _bus: &mut Bus) -> Outcome<SagaState, String> {
...
async fn test_saga_automated_rollback_with_state_mapping() {
    ...
    let step1 = SuccStep { id: "1".to_string() };
    let comp1 = CompStep { id: "1".to_string(), results: results.clone() };
    
    let step2 = SuccStep { id: "2".to_string() };
    let comp2 = CompStep { id: "2".to_string(), results: results.clone() };
    ...
    let initial_state = SagaState { cnt: 0, log: vec![] };
    let mut bus = Bus::new();
